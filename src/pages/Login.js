...
function Login() {
    ...
    function handleSubmit(e){
        ...
        if(email.length===0 || password.length===0){
            ...
            dispatch(login({email:email,password:password,role:'medical'}))
        }
    }
  ...
  return (
    ...
    <div className='w-full h-screen flex flex-col space-y-4 justify-center items-center -mt-32'>
        <div className="flex flex-col justify-center items-center">
            <p className="text-rose-800 font-bold text-7xl">MRS</p>
            <p className="text-rose-800 font-semibold">Medical Report System</p>
        </div>
        <form onSubmit={handleSubmit} className='flex flex-col rounded-lg border border-slate-400 w-80 h-auto space-y-1 p-2'>
            ...
            <button className='bg-rose-800 text-white py-1 px-2 rounded-lg w-fit' onClick={handleSubmit}>{loading?<>Loading</>:<>Login</>}</button>
        </form>
    </div>}
    </>
  )
